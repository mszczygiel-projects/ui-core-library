# @mszczygiel-projects/ui-core-icons
...
Named exports from `@mszczygiel-projects/ui-core-icons/react`. Each component accepts all standard `SVGProps<SVGSVGElement>` props (`className`, `style`, `aria-label`, etc.) and renders a scalable icon that inherits `color` from the parent via `currentColor`.

```tsx
import { IconAlert, IconSearch, IconUser } from '@mszczygiel-projects/ui-core-icons/react';
...
import type { IconName } from '@mszczygiel-projects/ui-core-icons';
...
```bash
pnpm --filter @mszczygiel-projects/ui-core-icons run build
```
...
```bash
pnpm --filter @mszczygiel-projects/ui-core-icons run test
```
...
- React exports are available from `@mszczygiel-projects/ui-core-icons/react`.
