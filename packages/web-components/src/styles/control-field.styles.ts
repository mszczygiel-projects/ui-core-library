...
export const controlFieldStyles = css`
  ...
  /*
   * The trailing chevron and clear button are actions, not field chrome, so
   * they take the action colour instead of --_icon. Variant-agnostic on
   * purpose: action/primary is identical for outline, filled and underlined.
   * Field state (success / error / disabled) still wins — those rules keep
   * using the --_icon-* family below.
   */
  :host {
